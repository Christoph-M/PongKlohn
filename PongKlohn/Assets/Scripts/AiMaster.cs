...
public class AiMaster : MonoBehaviour {
	...
	private Vector3 wantedTargetVector;
    private Vector2[] bounceArray;
	...
	{
		//bounceArray = ballTransform.gameObject.GetPath ();
		for (int i = 0; i <= (bounceArray.Length - 1); i++)
		{
			if(bounceArray[(i+1)] != null)
			{
				if((playerTransform.x <= bounceArray[i].x && playerTransform.x >= bounceArray[(i+1)].x) || (playerTransform.x >= bounceArray[i].x && playerTransform.x <= bounceArray[(i+1)].x) )
